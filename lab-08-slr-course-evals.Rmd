...
1.  Visualize the distribution of `score`.
    ...
    Why, or why not? ***The distribution is skewed, which means that many students are rating their courses positively. I would expect the ratings to be more evenly distributed. I assumed that there would be some sort of negative data since most students who do reviews are complaining about the professor.***
...
3.  Recreate the scatterplot from Exercise 2, but this time use\
    `geom_jitter()`? What does "jitter" mean? What was misleading about the initial scatterplot? ***Jitter is just a tool that adds a small amount of random variation to each point so it is easier to distinguish certain points that potentially overlap. The initial scatter plot was misleading because there were numerous points overlapping in the plot.***
...
***For an average beauty rating of 0 the average evaluation rating is 3.88. This doesn't make sense in the data as there are not beauty ratings that are 0.***
...
line corresponding to male professors?
...
***The average rating for a teaching rack professor is 4.28. The average rating for a tenure track professor is 0.12 points lower than a teaching track professor. The average rating for a tenured professor is 0.14 points lower than a teaching professor.***

12. Create a new variable called `rank_relevel` where `"tenure track"` is the baseline level.

```{r}
evals <- evals %>%
  mutate(rank_relvel = relevel(rank, ref="tenured"))
...
score_rank_relevel_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(score ~ rank_relvel, data=evals) %>%
  tidy()
...
***The average rating for a tenure professor is 4.14. The average rating for a teaching professor is 0.14 points higher than a tenured professor. The average rating for a tenure track professor is 0.02 points higher than a tenured professor.***
...
```{r}
evals <- evals %>%
  mutate(tenure_eligible = recode(rank, "tenure rack" = "yes", "tenured" = "yes", "teaching" = "no"))
```
